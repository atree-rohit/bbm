...
function renderMap() {
    ...
    var projection = d3.geoMercator().scale(250).center([78.9, 20.6]);
    ...
    places.forEach(p => {
        ...
        if (matchFlag == false) {
            ...
            h3Hex.push({ "hexID": h3Address, "value": p.pointCount, "coordinates": h3Geo });
        }
    });
    // console.log(h3Hex);
    ...
    h3Hex.forEach(h => {
        h3Hexes.data(h.coordinates.features)
            ...
            .on("click", (d) => { ons.notification.toast(` ${h.value} Observations`, { timeout: 2000, animation: 'lift' }) });
    });
...
function renderSpecies() {
    ...
    d3.select("body").append("svg")
        ...
        .append("g")
        .attr("transform", "translate(" + width / 2 + "," + (height / 2) + ")");
    document.getElementById('d3_table tbody').addEventListener('mouseover', 'tr', function() {
            this.css('cursor', 'pointer');
        })
        .on('click', 'tr', function() {
            var id = this.closest('tr').find('td').eq(0).html();
            var url = '/biodiversity/species/' + id;
            window.location = url;
        });
}
